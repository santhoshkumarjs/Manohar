...
//echo $product_sql = "select * from product where name like '%".$keyword."%' and status = 'active'";
$product_sql = "SELECT p.* FROM product AS p, category AS c WHERE p.categoryid = c.id AND (p.name LIKE '%".$keyword."%' OR c.name LIKE '%".$keyword."%') AND p.status = 'active' AND c.status = 'active'";
			...
			<div class="row mrg_0"><!-- marl6-->
				<h2 class="heading"><span>Art Works</span></h2>
					...
					<?php if($total_product == 0){  ?>
							<p class="subtitle fancy">No Match Found</p>
					<?php   }  ?>
