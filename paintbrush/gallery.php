<?php require 'framework/initialise/framework.init.php'; 
	  ...
	  $product_sql = "select count(*) as total from product where status = 'active'"; // and quantity != 0";	  
	  $product_data = $db['master']->getOneRow($product_sql);
	...
	<section class="sectiontop">
		<div class="container">
			...
			<div class="row mrg_0">
				...
				<!---<p class="subtitle fancy"><?php //echo $total; ?> ORIGINALS LISTED</p>--->
...
<script>
function changedetail(prodid, imgobj){
	$(".link-button").attr("href", "product.php?id="+prodid);
	quantity = imgobj.getAttribute("quantity");	
	if(quantity == 1)
		$(".sold").hide();
	else
		$(".sold").show();		
