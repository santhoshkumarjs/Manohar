! function(a, b, c) {
    ...
    a.fn.imagesLoaded = function(b) {
        ...
        function m(b, c) {
            b.src !== g && a.inArray(b, i) === -1 && (i.push(b), c ? k.push(b) : j.push(b), a.data(b, "imagesLoaded", {
                isBroken: c,
                src: b.src
...
$.fn.imagesLoaded = function(a) {
    ...
    function j(a, b) {
        a.src !== BLANK && $.inArray(a, f) === -1 && (f.push(a), b ? h.push(a) : g.push(a), $.data(a, "imagesLoaded", {
            isBroken: b,
            src: a.src
    ...
    $.elastic_grid = {
        ...
        function B() {
            C(n), s.on("debouncedresize", function() {
                q = 0, p = -1, A(), D();
        ...
        function C(a) {
            a.on("click", "span.og-close", function() {
                ...
                var b = $(this).parent();
                return b.removeClass("animate"), o === b.index() ? F($(this)) : E(b), !1
            })
        }
        ...
        function E(a) {
            ...
            p = c, b = $.data(this, "preview", new G(a)), b.open()
        }

        function F() {
            n.find(".og-pointer").remove(), o = -1;
            var a = $.data(this, "preview");
            "undefined" == typeof a || a.close(), $.removeData(this, "preview")
        }
        ...
        for (itemIdx = 0; itemIdx < c; itemIdx++)
            if (void 0 != a.items[itemIdx]) {
                ...
                strTag = strTag.substring(1), liObject.attr("data-tags", strTag), aObject = $("<a></a>"), aObject.attr("href", "javascript:;;"), imgObject = $("<img/>"), imgObject.attr("src", f.thumbnail[0]), imgObject.attr("data-largesrc", f.large[0]), imgObject.attr("alt", f.img_title[0]), imgObject.attr("title", f.img_title[0]), spanObject = $("<span></span>"), spanObject.html(f.title), captionobject=$('<div class="captiontext"></div>'), captionobject.html(f.captiontext), figureObject = $("<figure></figure>"), figureObject.append(spanObject), figureObject.append(captionobject), imgObject.appendTo(aObject), figureObject.appendTo(aObject), aObject.appendTo(liObject), liObject.appendTo(e)
            }
        ...
        var h = b.find("#portfolio-filter"),
            i = e.find("li"),
            j = {};
        numOfTag = 0, i.each(function(a) {
            var b = $(this),
                c = b.data("tags").split(",");
            b.attr("data-id", a), b.addClass("all"), $.each(c, function(a, c) {
                c = $.trim(c), b.addClass(c.toLowerCase().replace(" ", "-")), c in j || (j[c] = [], numOfTag++), j[c].push(b)
            })
        }), numOfTag > 1 ? (l(a.showAllText), $.each(j, function(a, b) {
            l(a)
        })) : h.remove(), h.find("a").bind("click", function(a) {
            m.find("li.og-expanded").find("a").trigger("click"), m.find(".og-close").trigger("click"), $this = $(this), $this.css("outline", "none"), h.find(".current").removeClass("current"), $this.parent().addClass("current");
        ...
        }), h.find("li:first").addClass("current");
        var t, m = e,
            n = m.children("li"),
            o = -1,
            p = -1,
            ...
            w = v[Modernizr.prefixed("transition")],
        ...
        G.prototype = {
            create: function() {
				...
				this.$detailButtonList), 
				this.$loading = $('<div class="og-loading"></div>'), 
				/* Below line is added by antony */
				this.$fullimage = $('<div class="og-fullimg"><div class="sold"><p>Sold</p></div></div>').append(this.$loading), 
				this.$closePreview = $('<span class="og-close"></span>'),
				this.$previewInner = $('<div class="og-expander-inner"></div>').append(this.$closePreview, this.$fullimage, this.$details), 
				this.$previewEl = $('<div class="og-expander"></div>').append(this.$previewInner), 
				this.$item.append($('<div class="og-pointer"></div>')),
				this.$item.append(this.getEl()), x && this.setTransition()
            },
            update: function(b) {
                if (b && (this.$item = b), o !== -1) {
                    var c = n.eq(o);
                    c.removeClass("og-expanded"), this.$item.addClass("og-expanded"), this.positionPreview()
                }
                if (o = this.$item.index(), "undefined" == typeof a.items[o]);
                else {
                    if (eldata = a.items[o], this.$title.html(eldata.title), this.$description.html(eldata.description), this.$detailButtonList.html(""), urlList = eldata.button_list, urlList.length > 0)
                    ...
                    if ("undefined" != typeof f.$largeImg && f.$largeImg.remove(), glarge = eldata.large, gthumbs = eldata.thumbnail, imgTitle = eldata.img_title, glarge.length == gthumbs.length && glarge.length > 0) {
                        var h = $("<ul></ul>");
                        for (g = 0; g < gthumbs.length; g++) {
							//alert(eldata.prodID[g]);
                            var i = $("<li></li>"),
                                e = $('<a href="javascript:;;"></a>'),
                                j = $("<img/>");
                            j.addClass("related_photo"), 0 == g && j.addClass("selected"), j.attr("src", gthumbs[g]), j.attr("data-large", glarge[g]), j.attr("quantity", eldata.quantity[g]), j.attr("alt", eldata.prodID[g]), j.attr("onclick", "changedetail(this.alt, this);"), j.attr("title", imgTitle[g]), e.append(j), i.append(e), h.append(i)
                        }
                        h.addClass("elastislide-list"), h.elastislide();
                        var k = $('<div class="elastislide-wrapper elastislide-horizontal"></div>');
                        k.append(h).find(".related_photo").bind("click", function() {
                            k.find(".selected").removeClass("selected"), $(this).addClass("selected"), $largePhoto = $(this).data("large"), $titlePhoto = $(this).attr("title"), $("<img/>").load(function() {
                                f.$fullimage.find("img").fadeOut(500, function() {
                        ...
                        }), f.$details.append('<div class="infosep"></div>'), f.$details.append(k)
                    } else f.$details.find(".infosep, .og-grid-small").remove();
                    f.$fullimage.is(":visible") && (this.$loading.show(), $("<img/>").load(function() {
                        var a = $(this);
                        a.attr("src") === f.$item.children("a").find("img").data("largesrc") && (f.$loading.hide(), f.$fullimage.find("img").remove(), f.$largeImg = a.fadeIn(350), f.$fullimage.append(f.$largeImg))
                    }).attr("src", eldata.large[0]))
					//Below condition was added by antony
					if(eldata.quantity[0] == 1)
						$(".sold").hide();
					else
						$(".sold").show();					
                }
            ...
            close: function() {
                var a = this,
                    b = function() {
                        x && $(this).off(w), a.$item.removeClass("og-expanded"), a.$previewEl.remove()
                    };
                return setTimeout($.proxy(function() {
                    "undefined" != typeof this.$largeImg && this.$largeImg.fadeOut("fast"), this.$previewEl.css("height", 0);
                    var a = n.eq(this.expandedIdx);
                    a.css("height", a.data("height")).on(w, b), x || b.call()
                }, this), 25), !1
            },
            calcHeight: function() {
                var a = t.height - this.$item.data("height") - r,
                    b = t.height;
                a < y.minHeight && (a = y.minHeight, b = y.minHeight + this.$item.data("height") + r), this.height = a, this.itemHeight = b
            },
            setHeights: function() {
                var a = this,
                    b = function() {
                        x && a.$item.off(w), a.$item.addClass("og-expanded")
                    };
                this.calcHeight(), this.$previewEl.css("height", this.height), this.$item.css("height", this.itemHeight).on(w, b), x || b.call()
            },
            positionPreview: function() {
                var a = this.$item.data("offsetTop"),
                    b = this.$previewEl.offset().top - q,
                    c = this.height + this.$item.data("height") + r <= t.height ? a : this.height < t.height ? b - (t.height - this.height) : b;
                u.animate({
                    scrollTop: c
                }, y.speed)
            },
            setTransition: function() {
                this.$previewEl.css("transition", "height " + y.speed + "ms " + y.easing), this.$item.css("transition", "height " + y.speed + "ms " + y.easing)
            },
            getEl: function() {
